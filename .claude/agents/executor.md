---
name: executor
description: Use when implementing code changes in hal-ti. Writes production code following all project constraints — no heap allocation, ISR safety, TI Tiva C register sequences (PRxxx polling, not NOP delays), ARM Cortex-M interrupt handling, vector table hygiene in both startup files, SOLID principles, and documentation alignment.
...
- Update `CLAUDE.md` Known Code Inconsistencies and `copilot-instructions.md` Common Pitfalls if a new failure mode is discovered
...
- Do NOT use `__asm("nop")` delays after `SYSCTL->RCGCxxx |=` — use `SYSCTL->PRxxx` polling
